...
import type { CareMemberDashboardData, MedicationAdherenceSummary, MedicationLogRecord, MedicationRecord, AppointmentRecord } from "@/lib/medic-types";
...
export default function FamilyDashboardPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch("/api/dashboard/family").then((r) => r.ok ? r.json() : null),
      fetch("/api/medication-logs").then((r) => r.ok ? r.json() : null),
  ...
  const summary = patient?.medicationSummary ?? { activeMedications: 0, dueToday: 0, takenToday: 0 };
  ...
  return (
    <main className="pd-page">
      <header className="flex justify-between items-start mb-5">
        <div>
          <h1 className="pd-heading">Welcome, {dashboard?.user?.firstName ?? "User"}!</h1>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <Link href="/family/profile"><div className="w-10 h-10 rounded-full border-2 border-[#2F3E34] flex items-center justify-center bg-[#E5E7EB]"><UserRound className="w-6 h-6 text-[#2F3E34]" /></div></Link>
        </div>
      </header>
        ...
        <section className="mb-4">
          ...
          <div className="flex gap-2">
            {dashboard!.activeLinkedPatients.slice(0, 2).map((p) => (
              <Link key={p.relationshipId} href={`/family/dashboard?patientId=${p.patientUserId}`}
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-3">
          ...
          {nextSlot && (
            <div className="pd-card p-4 cursor-pointer hover:opacity-90 transition" onClick={() => setSelectedMed(nextSlot.med)}>
      ...
      <nav className="pd-nav">
        <div className="pd-nav-active"><Link href="/family/dashboard" className="flex items-center justify-center w-full h-full"><House className="w-8 h-8" /></Link></div>
      ...
      {selectedMed && medModalData && (
        <MedicationViewModal data={medModalData} canEdit={false} onClose={() => setSelectedMed(null)} />
      )}
      {selectedAppt && apptModalData && (
        <AppointmentViewModal data={apptModalData} canEdit={false} onClose={() => setSelectedAppt(null)} />
      )}
    </main>
  );
}
